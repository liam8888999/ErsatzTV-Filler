...
then
  ...
  if [ ! -d $workdir/weatherv4 ]; then
    mkdir -p $workdir/weatherv4;
  fi
  ...
  find $workdir/weatherv4/ -name "*.mp4" -print > $scriptdir/v4.txt
  ffmpeg -y concat -i $workdir/v4.txt -i "$audio3" -shortest -c:a copy $workdir/weather-v4.mp4
