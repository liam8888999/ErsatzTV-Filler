...
# Generate Results
curl -s "$newsfeed" | xsltproc $newsstyle - | grep -v xml | sed 's/\&lt//g' | sed 's/\/p//g' | man -l - | col -bx > $workdir/newstemp.txt
...
ffmpeg -y -f lavfi -i color=$newsbackground1:$videoresolution -stream_loop -1 -i $audio3 -shortest -vf "drawtext=textfile='$workdir/news.txt': fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf: x=(w-text_w)/2:y=h-$textspeed*t: fontcolor=$newstextcolour1: fontsize=W/40:"  -pix_fmt yuv420p -c:a copy -t 00:01:00 $output/news-v1.mp4
