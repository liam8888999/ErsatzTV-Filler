...
fs.writeFileSync(`${WORKDIR}/news.txt`, wrappedText, 'utf8');
      ...
      const command = `ffmpeg -y -f lavfi -i color=white:${videoWidth}x720 -stream_loop -1 -i "/Users/liam/Documents/GitHub/ErsatzTV-Filler/audio-fallback/2.mp3" -shortest -vf "drawtext=textfile='${WORKDIR}/news.txt':x=(w-text_w)/2:y=h-40*t:fontcolor=black:fontsize=${textWidth}:box=1:boxcolor=white:boxborderw=5:line_spacing=6" -pix_fmt yuv420p -c:a copy -t 90 ${WORKDIR}/news-v1.mp4
`;
